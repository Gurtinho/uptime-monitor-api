...
describe('Monitor Repository', () => {
  ...
  it('should not create a monitor if the url already exists', async () => {
    await expect(monitorInMemoryRepository.create(id, monitor)).rejects.toThrow(
  ...
  it('should update a monitor successfully', async () => {
    ...
    expect(updatedMonitor.updatedAt.getTime()).toBeGreaterThanOrEqual(
      createdMonitor.createdAt.getTime()
    );
  });
  ...
  it('should not update a monitor if it does not exist', async () => {
    ...
    await expect(monitorInMemoryRepository.update('invalid-id', updateData)).rejects.toThrow(
      'Monitor not found'
    );
  });

  it('should not delete a monitor if it does not exist', async () => {
    await expect(monitorInMemoryRepository.delete('invalid-id')).rejects.toThrow(
      'Monitor not found'
    );
  });
});
